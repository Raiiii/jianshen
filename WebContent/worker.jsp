...
<%
	...
	int pagesize=5;
	String key="";
	int rcount=new WorkerServer().GetCount(key);
	
    List<Worker> ws=new WorkerServer().GetAll(startindex,pagesize,key);
  	...
  	<table cellpadding="0" cellspacing="0" style="width:1000px; margin:0px auto">
		...
		<c:forEach items="${ ws }" var="w">
			...
			<tr class="rows">
				...
				<td>${ w.address }</td>
				<td>
					<a href='pay.jsp?wid=${ w.id }'>指导记录</a>
					<a href='addworker.jsp?param=edit&&id=${ w.id }'>编辑</a>
					<a href='Ref/worker.jsp?param=del&&id=${ w.id }'>删除</a>
				</td>
			</tr>
		</c:forEach>
	</table>
