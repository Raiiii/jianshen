...
<%
    ...
    List<Equipment> ws=new EquipmentServer().GetLendAll(c.getId(),startindex,pagesize,key);
  	...
  	<table cellpadding="0" cellspacing="0" style="width:1000px; margin:0px auto">
		...
		<c:forEach items="${ ws }" var="w">
			...
			<tr class="rows">
				...
				<td>
				   <c:if test="${ w.remainNum==0 && w.lendNum==0}">已借光</c:if>
				  <c:if test="${ w.remainNum==0 && w.lendNum>0}"><a href='Ref/equipment_lend.jsp?param=huan&&id=${ w.id }'>还</a></c:if>
					<c:if test="${ w.remainNum>0 && w.lendNum==0}"><a href='Ref/equipment_lend.jsp?param=jie&&id=${ w.id }'>借</a></c:if>
					 <c:if test="${ w.remainNum>0 && w.lendNum>0}">
					<a href='Ref/equipment_lend.jsp?param=huan&&id=${ w.id }'>还</a> &nbsp;&nbsp;&nbsp;
					<a href='Ref/equipment_lend.jsp?param=jie&&id=${ w.id }'>借</a>
					 </c:if>
